...
def sun_zen_out(image, constants):
    set_image(image, 'centerLat',
              ee.Number(ee.Geometry(image.get('system:footprint')).bounds().centroid(30).coordinates().get(0)).multiply(
...
def brdf(image, band_name, kvol_band, kgeo_band, coefficients, constants):
    args = {**coefficients, 'kvol': '3 * i.' + kvol_band, 'kgeo': 'i.' + kgeo_band}  # Note the multiplication factor
